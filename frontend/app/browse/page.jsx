...
import {
  ...
  ChevronLeft, ChevronRight, Play, X, Code2, Trash2,
} from 'lucide-react';
...
function CollectionListItem({ name, count, active, onClick }) {
  return (
    <button
      onClick={onClick}
      className={cn(
    ...
    >
      ...
      <span className="text-xs text-text-muted">{typeof count === 'number' ? count.toLocaleString() : '—'}</span>
    </button>
  );
}
