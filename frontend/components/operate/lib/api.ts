// Same-origin: the Next.js dashboard proxies /api/* to the backend
// (see app/api/[...path]/route.js), so no cross-origin base is needed.
const BASE = ''

// One session for the whole product. CustomDB's login writes this key; the
// PivotDB pages read it, which is what makes a single sign-in cover both
// halves. (Upstream PivotDB used its own 'token' key and its own login page —
// both are gone.)
...
// NOTE: PivotDB's login/register/auth-status helpers used to live here. The
// merged product authenticates through CustomDB (/api/auth) and these pages
// are only ever reached behind that session, so they were removed rather than
// left as a second, divergent way to sign in.
