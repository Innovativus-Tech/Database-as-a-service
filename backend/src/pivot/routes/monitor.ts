import type {
  ShimApp as FastifyInstance,
  ShimReply,
} from '../adapter/fastify-express.js';
import type { Request as ShimRequest } from 'express';
...
export async function monitorRoutes(app: FastifyInstance) {
  ...
  const killHandler = async (req: ShimRequest, reply: ShimReply) => {
...
/**
 * Live current-ops stream: one Socket.IO namespace per connection id.
 *
 * `io` is the Socket.IO server the merged backend attaches to its HTTP
 * listener (see src/index.js) — upstream this came from Fastify's socket.io
 * plugin as `app.io`.
 */
export function registerMonitorSocket(io: SocketIoServer) {
  const ns = io.of(/^\/monitor\/.+$/);

  ns.on('connection', async (socket: SocketIoSocket) => {
